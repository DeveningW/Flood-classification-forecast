...
def evaluate(G, D, test_model, test_loader, TX_features, writer, epoch, total_step, tsne=False):
    ...
    for step, (raindrop, runoff_history, runoff, feature) in enumerate(test_loader):
        ...
        if C_features is None:
            C_features = ChangHua_feature.detach().cpu().numpy()
        else:
            C_features = np.concatenate([C_features, ChangHua_feature.detach().cpu().numpy()], axis=0)

    # feature visualization
    fig = plot_backbone_features(C_features.detach().cpu().numpy(), feature.cpu().numpy())
    writer.add_image("feature visualization", fig, epoch, dataformats='HWC')

    if tsne is True:
        start = random.randint(0, int(TX_features.shape[0] / 10))
        ChangHua_features = C_features.reshape((C_features.shape[0] * C_features.shape[1], C_features.shape[2]))
        for i in range(5):
            if i == 0:
...
def main(args):
    ...
    H_features = test_set.get_H_feature()
    ...
    return train(backbone, test_model, train_loader, test_loader, H_features, writer, save_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backbone', default='STGCN', type=str)  # RNN, LSTM, GRU, ANN, STGCN, TCN
    ...
    parser.add_argument('--pretrained_weights',default='', help='adj data path')
    parser.add_argument('--pre_backbone', default='STGCN', type=str)  # RNN, LSTM, GRU, ANN, STGCN, TCN
